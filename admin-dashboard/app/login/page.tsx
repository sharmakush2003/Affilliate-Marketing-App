...
const getLocationString = async (): Promise<string> => {
  ...
  return new Promise((resolve) => {
    navigator.geolocation.getCurrentPosition(
      ...
      (geoError) => {
        // Non-blocking fallback if user denies or OS blocks geolocation
        console.warn('Geolocation not shared or unavailable:', geoError?.message || 'Permission denied')
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.session) {
        // Resolve location gracefully without blocking
        const locationName = await getLocationString()
        ...
        try {
          ...
          console.warn('Session log error:', logErr)
        }
